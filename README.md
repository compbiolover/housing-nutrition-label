...
- **Per-dimension uncertainty / confidence display** *(next up)* — surface the uncertainty the models already carry (Infrastructure Burden ±30%; embodied carbon order-of-magnitude; climate scenario bands; several BRM bonuses flagged "v1 estimate"). A small confidence badge or error whisker per bar, plus a data-quality note driven by the `location_notes`/`*_data_source` the pipeline already stores. Highest-fit with the project's "flag what failed verification" ethos, and it strengthens every other feature — including the cost figure. See [research/lifetime-cost-research.md](research/lifetime-cost-research.md) §4.
...
- **Live address lookup on the label page** — wire the existing live location-resolution + scoring path into [`docs/label.html`](docs/label.html), which today renders a fixed set of Memphis construction presets, so a visitor can enter their own address instead of picking a preset.
...
**Shipped:**
- ~~"Cost over a mortgage" (lifetime cost of ownership) + comparison mode~~ → the label now present-values the two dollar-defensible flows — modeled **energy cost** and **expected annual disaster loss** — over a 15/30-year mortgage and shows the result as a **comparative delta vs. a typical comparable** at the same location (never an absolute "total cost"), mirroring the EPA fuel-economy sticker's "you save $X over 5 years" construction. Constant (real) dollars, no real escalation, discounted at ~4% real (homeowner mortgage opportunity cost) with an OMB ~2% social-rate band; the headline is rounded to 2 significant figures. A new **Compare (A/B)** mode puts two profiles side by side with a per-dimension delta table. The strip is fed by numeric `cost` fields now emitted per parcel by [`scripts/generate_label_data.py`](scripts/generate_label_data.py); no scoring/model change was required. Full methodology, discount-rate/escalation citations, and the dollarizable-vs-qualitative dimension audit: [research/lifetime-cost-research.md](research/lifetime-cost-research.md).
