...
| 4 | **Environmental Footprint** | Model/parametric, *heterogeneous by leg* | Operational leg = strongest (metered-equivalent kWh/therms × EPA eGRID2023 factors); embodied leg **explicitly flagged "LOW CONFIDENCE"** in `env_data_source` (sparse US single-family benchmarks, order-of-magnitude); water leg locally favorable. eGRID **vintage** stored. |
...
| **T — Recency / temporal** | Current vintage (eGRID2023, ACS 5-yr latest, PLACES latest) | Recent but static reference (ResStock, service-life tables) | Dated or undated constant | vintage strings in `*_data_source` |
...
| Environmental Footprint | 1 overall (op leg strong, **embodied leg flagged LOW**) | 2 (parcel) | 2 (eGRID2023) | 1 (embodied order-of-magnitude) | **6** | **Moderate** | `env_data_source` literally carries *"LOW CONFIDENCE"* on embodied — hold at Moderate. |
