...
# Postal → 2-digit state FIPS. EIA publishes the 50 states + DC only, so Puerto
# Rico is dropped from the shared table rather than emitting a row with no data.
FIPS = {u: f for u, f in USPS_TO_STATE_FIPS.items() if u != "PR"}
